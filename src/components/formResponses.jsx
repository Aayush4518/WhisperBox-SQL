...
export default function FormResponses() {
  ...
  useEffect(() => {
    const fetchFormAndResponses = async () => {
      try {
        const formRes = await fetch(`http://localhost:5000/forms/${formId}`);
        if (!formRes.ok) throw new Error(`HTTP error! status: ${formRes.status}`);
        ...
        const responsesRes = await fetch(`http://localhost:5000/forms/${formId}/responses`);
        if (!responsesRes.ok) throw new Error(`HTTP error! status: ${responsesRes.status}`);
  ...
  const getAnswerText = (questionId, response) => {
    ...
    <div className="relative w-full min-h-screen flex items-center justify-center mt-10 mb-10 px-4">
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="p-10 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-[0_0_30px_rgba(168,85,247,0.3)] mb-8">
          ...
          <p className="text-white/70 mb-2">{form.description}</p>
          <p className="text-white/60 text-sm mb-2">Form ID: {form.form_id}</p>
                ...
                <div className="space-y-4">
                  {form.questions.map((question) => (
                    <div key={question.id} className="border-l-4 border-purple-500 pl-4 py-2">
                      ...
                      <p className="text-white/80 bg-white/5 p-3 rounded">{getAnswerText(question.id, response)}</p>
                    </div>
                  ))}
                </div>
              </div>
